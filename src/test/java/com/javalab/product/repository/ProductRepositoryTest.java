...
public class ProductRepositoryTest {
	...
	*/
	 ...
	 *  Product 한개 조회
	 *  [Fetch 전략 - Eager / Lazy]
	 *   - Fetch 전략을 Lazy로 하게되면 지연로딩이 되어 
	 *     처음에는 상품테이블만 조회하고 카테고리 테이블이 필요한 시점
	 *     즉, product.toString()이 되는 시점에
	 *     카테고리 테이블에 조회해서 카테고리 관련 모든 정보를 조회한다.
	 *   - Fetch 전략을 안주거나, Eager로 하게 되면
	 *     처음부터 상품테이블과 카테고리 테이블을 조인걸게 된다.
	 *     그렇게 되면 데이터베이스 성능에 좋지않다.
	 *     가급적이면 지연로딩을 사용하는게 낫다
	 *   - @ManyToOne : 기본이 즉시로딩(Eager)
	 *   - @OneToMany : 기본이 지연로딩(Lazy)
	 */
	//@Test
	//@Disabled
	public void testGetProductById2() {
		Optional<Product> optProduct = productRepository.findById(1); // 데이터베이스 확인
		//조회결과가 null이면 오류가 나기때문에 Optional 사용하여 값이 없어도 오류나지 않게함
		Product product = optProduct.get();
		log.info("Product 의 이름조회 : " + product.getProductName());
		
		// 1. 상품 엔티티의 Category 멤버변수에 지연로딩을 설정하게되면 이 시점까지는 상품 테이블만 조회함.
		// 2. 아래의 produt.toString()으로 상품 정보의 
		//    모든 내용(Category포함)을 보겠다고 하는 시점에 그제서야 카테고리 테이블에 조회를 한다.
